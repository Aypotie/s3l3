...
generate_all() {
    ...
    lcov --capture --directory . --output-file coverage.info --include "*/data_structures/*" 1>/dev/null
...
generate_one() {
    ...
    echo -e "${BLUE}Generating coverage report:${RESET} $test_file"
    lcov --capture --directory . --output-file coverage.info --include "*/data_structures/*"
